...
export default function StoreCatalog({ userId, onBack }: StoreCatalogProps) {
    ...
    setCart(prev => {
      return prev.map(i => {
        if (i.variantId === variantId) {
          ...
          const variant = product?.varian.find(v => v.id === variantId);
          const minOrder = variant ? Math.max(1, Number(variant.min_order ?? product?.min_order) || 1) : Math.max(1, Number(product?.min_order) || 1);
  ...
  const openDetail = (product: Product) => {
    ...
    setDetailQty(initialVariant ? Math.max(1, Number(initialVariant.min_order ?? product.min_order) || 1) : Math.max(1, Number(product.min_order) || 1));
    setDetailNote('');
  };
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {selectedProduct.varian.filter(v => v.harga_jual > 0).map(v => {
                          ...
                          return (
                            <button key={v.id} onClick={() => { setSelectedVariantId(v.id); setDetailQty(Math.max(1, Number(v.min_order ?? selectedProduct.min_order) || 1)); }}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => setDetailQty(q => {
                      const minOrder = Math.max(1, Number(selectedProduct.varian.find(v => v.id === selectedVariantId)?.min_order ?? selectedProduct.min_order) || 1);
    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        ...
        <div className="flex items-center gap-2 flex-wrap mt-0.5">
          ...
          {(() => {
            const minOrder = Math.min(...sellable.map(v => Number(v.min_order ?? product.min_order) || 1));
            if (minOrder > 1) {
              return (
